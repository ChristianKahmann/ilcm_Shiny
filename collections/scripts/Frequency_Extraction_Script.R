...
error<-try(expr = {
  ...
  if(parameters$fe_interview_use_speaker==TRUE){
    log_to_file(message = "<b>Apply Speaker Restriction</b>",file = logfile)
    if(length(parameters$fe_interview_speaker_filter)>0){
      dataset = info[[2]][1,1]
      id_docs = info[[1]][,1]
      ...
      speaker_ids <- which(grepl(x = data_interview$sprecher,pattern = stringr::regex(paste0("(",parameters$fe_interview_speaker_filter,")",collapse="|"))))
