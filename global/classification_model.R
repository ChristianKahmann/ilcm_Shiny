...
liblinear_model <- R6Class(
  ...
  private = list(
    ...
    transform_input = function(input_matrix) {
      if(max(input_matrix)==0){
      return(as.matrix.csr(as.matrix(input_matrix)))
      }
      else{
  ...
  public = list(
    set_input = function(input_features, gold) {
      ...
      #alt<-input_features
      #alt[which(alt[,2]=="taz_592541_3"),3]
...
#Create sparse M Format
    ...
    predict = function(ref_id = NULL,save_output=F){
      if(length(ref_id)!=0){

        
        #TFIDF on statistics from training data
        #Must training separated DTM be used to infer statistic for tfidf
        docs <- rownames(private$internal_representation[rownames(private$internal_representation) %in% ref_id,,drop=FALSE])
        
        internal <- private$internal_representation[docs, ,drop=FALSE]
        ...
        #DELETE ALL EXAMPLES WITH LESS THAN N FEATURE
        internal <- 
          internal[rowSums(internal) >=1, ,drop=F]
        target_matrix[rownames(internal), features] <- internal[,features,drop=FALSE]
...
cmodel <- R6Class(
  ...
  public = list(
    ...
    predict = function(ref_id = NULL,save_out=FALSE)
    {
      if(is.null(ref_id))
      {
        gold <- private$classify_machine$get_gold()
      ...
      private$classify_machine$predict(ref_id,save_out)
    },
    ...
    active_learn_examples = function(strategy = "LC", batch_size = 25, positive_class = NULL, labels = NULL, verbose = T, coder = "DEFAULT", save_out=FALSE)
    {
      gold <- private$classify_machine$get_gold()
    ...
    active_learning = function(
      ...
      while (!stop_active_learning) {
        ...
        oracle <- self$active_learn_examples(positive_class = positive_class, strategy = strategy, labels = labels, batch_size = batch_size, coder = coder,save_out=save_out)
        if(save_out==TRUE){
          return(oracle)
        }
