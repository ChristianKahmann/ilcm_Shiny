...
calculate_sentiments_analysis_tokens_object<-function(){
  ...
  tow<-tmca.util::TextObjectWrapper$new()
  #set log destinationget_token_meta_and_language_from_db<-function(get_meta=T,get_language=T,get_global_doc_ids=F){
  token<-NULL
  meta=NULL
  language=NULL
  global_doc_ids<-NULL
  ...
  return(list(token=token,meta=meta,language=language))
}
