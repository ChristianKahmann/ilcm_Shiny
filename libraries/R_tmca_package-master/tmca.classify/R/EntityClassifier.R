...
############################################################################################################################
#' An Abstraction 'base class' for  NER classification algorithms.
#'
...
#' To implement a new Algorithm inherit from this class and implement the methods call(), input(), output() and classify().
#'
#' @format R6Class
...
#' }
#'
EntityClassifier_abstr <- R6Class(
  "EntityClassifier_abstr",
  private = list(
    name = "",
    package = "",
...
####################################################################################
#' Implementation of BiLSTM-CRF algorithm
#' 
...
bilstm_crf_model <- R6Class(
  "bilstm_crf_model",
  inherit = classifier_abstr,
  private = list(
    name = "model",
    package = "bilstm_crf_model",
    parameters = list(
      cost = 0.1, 
      epsilon = 0.01, 
      bias = 1,
...
classify.register = list("tmca_classify::bilstm_crf_model" = bilstm_crf_model
)
...
##############################################################
#' A classifier Class
#' @description An R6 class to execute, evaluate and refine a NER classifier independently of the specific underlying algorithm.
#'              This uses an implementation of an algorithm as specified in the documentation of EntityClassifier_abstr as engine.
...
nercmodel <- R6Class(
  ...
  public = list(
    ...
    train = function(ref_id = NULL)
    {
      ...
      private$classify_machine$train(ref_id)
    },
