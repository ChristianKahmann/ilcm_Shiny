...
liblinear_model <- R6Class(
  ...
  public = list(
    ...
    train = function(ref_id = NULL, positive_class = NULL, from_dictionary_hits = F) {
      ...
      gold_table <- data.table::rbindlist(private$internal_gold$gold[select_query])
      
      labels <- as.character(gold_table$class)
    ...
    predict = function(ref_id = NULL){
      ...
      target_matrix[,features] <- internal[,features]
      ...
      fValues <- sapply(cParameterValues, function(x){
        ...
        for(fold in folds)
        {
          ...
          fold_score <- tmca.util::F.measure(
            ...
            inLabels = data.table::rbindlist(private$internal_gold$gold[private$internal_gold$ref_id %in% test])$class
          )
...
cmodel <- R6Class(
  ...
  public = list(
    ...
    query_oracle_human = function(ref_id, labels, positive_class, coder = "DEFAULT") {
      ...
      decisions <- factor(rep(0, length(ref_id)), levels = unique(labels))
      ...
      while(d <= length(ref_id))
      {
        ...
        if (!is.null(viewer)) {
          ...
          {
            doc_html <- paste0(doc_html, "<br/>",as.character(private$corpus$text[doc_id_idx + 1]))
          }
          
          if(doc_id_idx > 1)
          {
            doc_html <- paste0(as.character(private$corpus$text[doc_id_idx - 1], "<br/>",doc_html))
          }
          ...
          final_html <- paste0(final_html, "<p><b>", doc_html , "</b></p>")
        ...
        my_decision <- self$read_human_decision(labels = levels( decisions),positive_class = positive_class)
        ...
        {
          decisions[d] <- list(class = levels(decisions)[my_decision], coder = coder, timestamp = Sys.time())
          d <- d + 1
        }
