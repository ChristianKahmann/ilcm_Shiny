...
getVolForTermLogVarTS <- function(x, span = Inf)
{
  ...
  for(term in x)
  {
    ...
    for(i in span:ncol(volatilityRanks))
    {
      ...
      volatility <- mean(apply(tmp, 1, rankQuantile))#??nzvar?
...
calculate_volat<-function(coocsYears,h,method,vocabIns,datesTmp,terms)
{
  ...
  for (j in 1:h)
  {
    data[j]<-Matrix(coocsYears[[(j+i-1)]])
    print("apply Ranks")
    count<-count-1
    ...
    print("vor switch")
    switch(method,
           no_zero = data_ranked[j,,]<-apply_Rank(as.matrix(data[j,,]),method,indices,terms),            
           max_rank = data_ranked[j,,]<-apply_Rank(as.matrix(data[j,,]),method,indices,terms), 
           recommender= data_ranked[j,,]<-apply_Rank(as.matrix(data[j,,]),method,indices,terms),
           abs_min_abs_max={ ergeb<-apply_Rank((data[[j]]),method,indices,terms)
  ...
  if(method=="abs_min_abs_max")
  {
    ...
    cost_Matrix<-berechneAbstand(data_ranked,terms)
    print("calc mean")
...
apply_Rank<-function(Term_Term_Matrix,method,indices,terms)
{
  switch(method,
         ...
         Term_Term_Matrix[which(Term_Term_Matrix==0)]<-NA
         pb_max<-txtProgressBar(min = 1,max=dim(Term_Term_Matrix)[1],style = 3,width = 50,title = "max_Rank")
         for (i in 1:length(vocabIns))
         {
           Rank_Matrix[i,]<-rangDecr(Term_Term_Matrix[i,])
         }
         RTT_ad<-Rank_Matrix
         for (i in 1:dim(Term_Term_Matrix)[1])
         { setTxtProgressBar(pb_max,i)
         ...
         Term_Term_Matrix<-adjust_Matrix(Term_Term_Matrix,indices)
...
adjust_Matrix<-function(Rank_Matrix,indices)
{
  Matrix_adjusted<-Rank_Matrix
  pb2<-txtProgressBar(min = 1,max=dim(Rank_Matrix)[1],style = 3,width = 50)
...
berechneAbstand<-function(testarray,terms)
{
  ...
  {
      c<-c+1
    for(j in 1:dim(testarray[[1]])[1])
    {
      Werte<-testarray[[1]][which(vocabIns==term),j]
      for( k in 2:length(testarray))
      {
        Werte<-c(Werte,testarray[[k]][which(vocabIns==term),j])
      }
