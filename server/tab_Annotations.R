...
output$annotations<-renderDataTable({
  ...
  dt<-datatable(data.frame(data, Delete = shinyInput(
    shinyBS::bsButton,
    dim(data)[1],
    ...
    style="danger",
    icon=icon("delete")
  )),rownames = F,escape = F,selection = "single",class = "row-border compact",options = list(columnDefs=list(list(className="no_select",targets=13))),
...
observeEvent(input$annotations_rows_selected,{
  ...
  if (length(s)) {
    ...
    values$Anno_id_global<-as.integer(values$annotations_all[s[length(s)],"global_doc_id"])
    values$Anno_collection<-(values$annotations_all[s[length(s)],"collection"])
