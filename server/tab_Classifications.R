  ...
  return(tagList(
    ...
    tabsetPanel(
      tabPanel(title = "one category for multiple examples",tagList(
        uiOutput(outputId = "Class_classifications"),
        uiOutput(outputId = "Class_evaluated_labels")
      )
      ),
  ...
  checkboxes<-lapply(rownames(highlight),function(x){
    ...
    return(as.character(tagList(prettyRadioButtons(inputId = radio_id,label = "",choiceValues = c("approve","deny","ignore"),choiceNames = list(tags$span(style = "color:#00b241;font-weight: bold;", "approve"),tags$span(style = "color:#f2342e;font-weight: bold;", "deny"),
                                ...
                                conditionalPanel(condition=condition,
                                                 shinyWidgets::prettyRadioButtons(inputId = paste0(radio_id,"_deny"),label = "other category?",choices = c("unknown",setdiff(values$Class_all_documents_annos[,1],highlight[x,2])),
...
#####one category######
output$Class_classifications<-renderUI({
  values$Class_update_classifications
  files<-list.files(path = paste0("collections/results/classification/activeLearning/",input$project_selected),full.names = T,include.dirs = F,recursive = T)
  validate(
  ...
  for(i in 1:length(files)){
    load(files[i])
    evaluated<-paste(length(which(data[,"approved"]==TRUE)),length(which(data[,"denied"]==TRUE)),dim(data)[1],sep="/")
    information<-c(as.character(learning_meta$date),as.character(learning_meta$color),learning_meta$category,paste("collection:",learning_meta$collection),evaluated)
    class<-rbind(class,information)
    class[i,2]<-paste0('<b style="background-color:',class[i,2],';">',class[i,2],'</b>')
  }
  colnames(class)<-c("date","color","category","classification","evaluated")
...
observe({
  ...
  load(list.files(path = paste0("collections/results/classification/activeLearning/",input$project_selected),full.names = T,include.dirs = F,recursive = T)[as.numeric(button_input[3])])
  data$other<-as.character(data$other)
  values$Class_eval_data<-data[1:20,]
  values$class_eval_data_index<-20
  values$Class_eval_data_complete<-data
  values$Class_eval_save_path<-list.files(path = paste0("collections/results/classification/activeLearning/",input$project_selected),full.names = T,include.dirs = F,recursive = T)[as.numeric(button_input[3])]
...
output$Class_eval_examples<-renderUI({
  examples<-lapply(X = 1:dim(values$Class_eval_data)[1],FUN = function(x){
    radio_id<-paste0("Class_eval_radio_buttons_",rownames(values$Class_eval_data)[x])
    condition<-paste0('input.',radio_id,'=="deny"')
    tagList(
      ...
      fluidRow(
        ...
        column(7,offset = 1,
               ...
               conditionalPanel(condition = condition,
                                shinyWidgets::prettyRadioButtons(inputId = paste0(radio_id,"_deny"),label = "other category?",choices = c("unknown",setdiff(values$class_anno_names,values$class_anno_selected)),
...
observeEvent(input$Class_eval_random,{
  values$Class_eval_data<-values$Class_eval_data_complete[sample(x = 1:50,size = 20,replace = FALSE),]
})

observeEvent(input$Class_eval_highest_scored,{
  if(values$class_eval_data_index==50){
    values$class_eval_data_index<-0
  }
  idx<-(values$class_eval_data_index+1):(values$class_eval_data_index+20)
  values$Class_eval_data<-values$Class_eval_data_complete[idx,]
  values$class_eval_data_index<-values$class_eval_data_index+20
})
...
observe({
  ...
  x<-unlist(lapply(rownames(values$Class_eval_data),function(i){
    if(is.null(input[[paste0("Class_eval_radio_buttons_",i)]])){
      return(" ")
    }
    else{
      return(input[[paste0("Class_eval_radio_buttons_",i)]])
    }
  }))
  y<- unlist(lapply(rownames(values$Class_eval_data),function(i){
    if(is.null(input[[paste0("Class_eval_radio_buttons_",i,"_deny")]])){
      return(" ")
    }
    else{
      return(input[[paste0("Class_eval_radio_buttons_",i,"_deny")]])
    }
  }))
...
observe({
  ...
  doc_id<-as.numeric(stringr::str_split(string = input$Class_eval_doclink,pattern = "_",simplify = T)[5])
...
#evaluated labels
############################################################
output$Class_evaluated_labels<-renderUI({
  values$Class_update_classifications
  files<-list.files(path = paste0("collections/results/classification/activeLearning/",input$project_selected),full.names = T,include.dirs = F,recursive = T)
  validate(
  ...
  for(i in 1:length(files)){
    load(files[i])
    class<-rbind(class,cbind(cbind(learning_meta$color,cbind(learning_meta$category,data)),files[i]))
  }
  ...
  categories<-unique(class[,2])
  df<-NULL
  for(i in categories){
    subclass=class[which(class[,2]==i),]
...
observe({
  ...
  x<-isolate(values$Class_eval_class)[which(as.character(isolate(values$Class_eval_class[,2]))==button_input[2]),]
...
observe({
  ...
  isolate(values$Class_eval_class[rownames(values$Class_eval2_data),5:7]<-FALSE)
  isolate(values$Class_eval_class[approved,5]<-TRUE)
  isolate(values$Class_eval_class[denied,6]<-TRUE)
  isolate(values$Class_eval_class[ignored,7]<-TRUE)
  if(any(sapply(X = y,FUN = nchar)>1)){
    isolate(values$Class_eval_class[other,8]<-y)
  }
})
...
output$Class_eval2_document<-renderUI({
  ...
  identifier<-stringr::str_split(string = values$Class_eval_class[as.character(values$Class_eval2_doc_id),3],pattern = "_",simplify = T)
  mydb <- RMariaDB::dbConnect(RMariaDB::MariaDB(), user='root', password='ilcm', dbname='ilcm', host=values$host,port=values$db_port)
  token<-RMariaDB::dbGetQuery(mydb, paste("select * from token where dataset='",identifier[1,1],"' and id=",identifier[1,2],";",sep=""))
  meta<-RMariaDB::dbGetQuery(mydb, paste("select title,date from documents where dataset='",identifier[1,1],"' and id_doc=",identifier[1,2],";",sep=""))
  RMariaDB::dbDisconnect(mydb)
...
observeEvent(input$Class_eval2_save,{
  withBusyIndicatorServer("Class_eval2_save", {
    data_class<-isolate(values$Class_eval_class)
    for(i in unique(data_class[,9])){
      i<-as.character(i)
      subset<-data_class[which(data_class[,9]==i),]
      load(i)
