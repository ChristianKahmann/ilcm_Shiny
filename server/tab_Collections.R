...
output$collections <- DT::renderDataTable({
  ...
  collection_names<-stringr::str_replace_all(
    string =  list.files("collections/collections/"),
    pattern = ".RData",
    replacement = ""
  )
  mydb <- RMariaDB::dbConnect(RMariaDB::MariaDB(), user='root', password='ilcm', dbname='ilcm', host=host,port=db_port)
  queries=RMariaDB::dbGetQuery(mydb,paste0("SELECT name,query FROM ilcm.Collections where name in ('",paste0(collection_names,collapse="', '"),"');"))
  
  RMariaDB::dbDisconnect(mydb)
  queries<-queries[order(queries[,1]),2]
  #collection_names<-sort(collection_names)
  values$collection_names<-collection_names
  collection_names<-unlist(lapply(1:length(collection_names),FUN = function(x){
    return(as.character(tags$span(id=paste0("collection_name_",x),collection_names[x])))
  }))
  if(length(queries)!=length(collection_names)){
    queries<-rep("unknown",length(collection_names))
  }
  df <- reactiveValues(
    data = data.frame(
  ...
  DT::datatable(
    ...
    selection = 'single',
    escape = FALSE,
    options = list(dom = 'tp',ordering=F,columnDefs=list(list(className="no_select",targets=c(1,2)),
