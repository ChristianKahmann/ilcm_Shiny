...
values$annotations_marked<-matrix(c(0),0,12)
...
output$document<-renderUI({
  ...
  if(isolate(is.null(isolate(values$new)))){
    ...
    annotations<-RMariaDB::dbGetQuery(mydb, paste("select * from Annotations where id='",(values$token)[1,2],"'",
                                                  " and trim(dataset)='",isolate(values$token)[1,1],"';",sep = ""))
    RMariaDB::dbDisconnect(mydb)
    annotations<-annotations[which(annotations[,"Anno_set"]==input$anno_scheme_selected),1:12]
    ...
    if(!is.null(isolate(input$anno_tag))){
      ...
      if(!is.null(isolate(input$anno_start))){
        for(k in isolate(input$anno_start):isolate(input$anno_end)){
          isolate(values$annos<-rbind(isolate(values$annos),c(k,name,values$user,color,input$anno_scheme_selected,id)))
        }
        indi<-isolate(input$anno_start):isolate(input$anno_end)
        new_values<-c(id,values$user,isolate(values$meta$dataset),isolate(values$selected),isolate(input$anno_start),isolate(input$anno_end),name,color,as.character(Sys.time()),isolate(input$anno_scheme_selected),"no_collection",as.numeric(isolate(values$meta)[1,"id"]))
