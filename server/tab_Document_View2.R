...
values$Doc_annotations_marked<-matrix(c(0),0,12)
values$Doc_annotations_show<-matrix(c(0),0,12)



#render the document
output$Doc_document<-renderUI({
  values$Doc_token[1,1]
  validate(
      need(
  ...
  if(isolate(is.null(isolate(values$Doc_new)))){
    ...
    annotations<-RMariaDB::dbGetQuery(mydb, paste("select * from Annotations where id='",(values$Doc_token)[1,"id"],"'",
                                                  " and trim(dataset)='",values$Doc_token[1,"dataset"],"';",sep = ""))
    RMariaDB::dbDisconnect(mydb)
    annotations<-annotations[which(annotations[,"Anno_set"]==input$Doc_anno_scheme_selected),1:12]
    ...
    if(!is.null(isolate(input$Doc_anno_tag))){
      ...
      if(!is.null(isolate(input$Doc_anno_start))){
        ...
        new_values<-c(id,values$user,isolate(values$Doc_meta$dataset),isolate(values$Doc_selected),isolate(input$Doc_anno_start),isolate(input$Doc_anno_end),
                      name,color,as.character(Sys.time()),isolate(input$Doc_anno_scheme_selected),values$current_collection,as.numeric(isolate(values$Doc_meta)[1,"id"]))
...
observeEvent(input$Doc_save_annotations,{
  if(dim(isolate(values$Doc_annotations_marked))[1]==0){
    ...
    for(i in 1:dim(isolate(values$Doc_annotations_marked))[1]){
      vals<-paste0(vals,'("',isolate(values$Doc_annotations_marked[i,1]),'",','"',isolate(values$Doc_annotations_marked[i,2]),'"',',','"',
                   isolate(values$Doc_annotations_marked[i,3]),'",',isolate(values$Doc_annotations_marked[i,4]),',',isolate(values$Doc_annotations_marked[i,5]),',',
                   '"',isolate(values$Doc_annotations_marked[i,6]),'"',',','"',isolate(values$Doc_annotations_marked[i,7]),'"',',','"',
                   isolate(values$Doc_annotations_marked[i,8]),'"',',','"',isolate(values$Doc_annotations_marked[i,9]),'"',',','"',
                   isolate(values$Doc_annotations_marked[i,10]),'"',',','"',isolate(values$Doc_annotations_marked[i,11]),'"',' , ',
                   isolate(values$Doc_annotations_marked[i,12]),' ,"',
                   paste(isolate(values$Doc_token)[isolate(values$Doc_annotations_marked)[i,5]:isolate(values$Doc_annotations_marked[i,6]),5],collapse=" "),'"),')
    }
