...
output$collection_documents<-DT::renderDataTable({
  ...
  if(input$Doc_row_sel>=1){
    if(values$Doc_custom==TRUE){
      ...
      validate(
        need(dim(ind)[2]>1,message="no result found"),
        need(dim(ind)[1]>1,message="no results found"))
      colnames(ind)<-stringr::str_replace_all(string = colnames(ind),pattern = "_[a-z]+$",replacement = "")
    }
  ...
  if(values$Doc_custom==TRUE){
    ...
    data<-datatable(data.frame(data,keep=shinyInput_checkbox_Doc(checkboxInput,dim(data)[1],"Doccbox_",values=!(data[,1]%in%isolate(values$delete_documents)),label=NULL))
    ...
    if(!is.null(isolate(input$sort))){
      ...
      data<-datatable(data.frame(data,keep=shinyInput_checkbox_Doc(checkboxInput,dim(data)[1],"Doccbox_",values=!(data[,1]%in%isolate(values$delete_documents)),label=NULL)),selection = "single",rownames = FALSE,escape = F,options = list(
        preDrawCallback = JS('function() { Shiny.unbindAll(this.api().table().node()); }'),
        drawCallback = JS('function() { Shiny.bindAll(this.api().table().node()); } '),
        dom="t",
        order=list(2,"desc"),
...
output$Documents_row<-renderUI({
  ...
  load(list.files("collections/collections/", full.names = T)[[input$collections_rows_selected]])  
  sliderInput(inputId = "Doc_row_sel",label = NULL,min = 1,max = ceiling(values$numFound_Sub/10),value = 1,step = 1)
})
