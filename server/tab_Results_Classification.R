...
output$Classification_Results <- renderDataTable({
  ...
  if(dim(parameters)[1]>0){
    ...
    data_finished<-data_finished[,c("task id","collection","creation.time","Project","Mode","Category","positive.Threshold","SVM_C","blacklist","whitelist")]
  }
  else{
    values$tasks_class<-data_finished
  }
  ...
  try({
    colnames(data_finished)<-c("Task id","Collection","Creation time","Project","Mode",
                               "Category","positive Threshold","SVM C","Blacklist","Whitelist")
  })
