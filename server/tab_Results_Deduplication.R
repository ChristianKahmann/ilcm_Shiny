...
output$Deduplication_Results <- renderDataTable({
  ...
  if(dim(parameters)[1]>0){
    ...
    data_finished<-data_finished[,c("task id","collection","creation.time","DD.similarity.measure","DD.threshold","DD.strategy")]
  }
  else{
    values$tasks_deduplication<-data_finished
  }
  ...
  try({
    colnames(data_finished)<-c("Task id","Collection","Creation time","Similarity measure","Threshold","Strategy","Delete","More details")
  })
