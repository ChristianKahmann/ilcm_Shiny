...
#' depends on:
#'    values$reload_dynamic_topic_result: reload topic model results
...
output$Dynamic_Topic_Results <- renderDataTable({
  ...
  isolate(values$reload_dynamic_topic_result<-FALSE)
  
  files <- list.files("collections/results/dynamic-topic-model/")
  validate(
    need(length(files)>0,"no results found")
  )
  files_for_date <-
    list.files("collections/results/dynamic-topic-model/", full.names = T)
  #get parameters
  ...
  values$Dynamic_Topic_Results_Files<-files
  ...
  values$tasks_dtm<-data_finished

  available_parameters<-intersect(c("task.id","collection","creation.time","tm_number_of_topics","dtm_split_how","remove_stopwords","termfreq_type","min_term","max_term","blacklist","whitelist"),colnames(data_finished))
  ...
  data_finished<-cbind(data_finished,Delete = shinyInput(
    shinyBS::bsButton,
    dim(data_finished)[1],
    'delete_button_dynamic_topic_results_',
    label = "",
    size="small",
    style="danger",
    icon=icon("trash"),
    onclick = 'Shiny.onInputChange(\"delete_dynamic_topic_results\",  this.id)'
  ))
  #more details buttons
  data_finished<-cbind(data_finished,More_Details = shinyInput(
    shinyBS::bsButton,
    dim(data_finished)[1],
    'more_details_button_dynamic_topic_results_',
    label = "",
    size="small",
    style="primary",
    icon=icon("info"),
...
#' depends on:
#'   input$Dynamic_Topic_Results_rows_selected: selected rowa from topic model result-list
#'   values$Details_Analysis: details of selected analysis
#'   values$Details_Data_DTM: details from topic model data
...
#' depends on:
#'   input$delete_dynamic_topic_results: deleted topic model results
#'   values$Dynamic_Topic_Results_Files: topic model result files
#'   values$dtm_selected_row: selected rows from topic model
#'   values$reload_dynamic_topic_result: reload topic model result
observeEvent(input$delete_dynamic_topic_results, {
  selectedRow <-
    as.numeric(strsplit(input$delete_dynamic_topic_results, "_")[[1]][6])
  if(selectedRow>0){
    unlink(values$Dynamic_Topic_Results_Files[selectedRow],recursive = T)
    shinyjs::useShinyjs()
    isolate(shinyjs::runjs('Shiny.onInputChange(\"delete_dynamic_topic_results\",  "delete_button_dynamic_topic_results_0")'))
...
#' depends on:
#'   input$more_details_dynamic_topic_results: more details of topic model results
#'   values$dtm_selected_row: selected row from topic model result list
observeEvent(input$more_details_dynamic_topic_results,{
  selectedRow <-
    as.numeric(strsplit(input$more_details_dynamic_topic_results, "_")[[1]][7])
  if(selectedRow>0){
    values$dtm_selected_row<-selectedRow
    isolate(shinyjs::runjs('Shiny.onInputChange(\"more_details_dynamic_topic_results\",  "more_details_button_dynamic_topic_results_0")'))
    showModal(
      modalDialog(easyClose = T,fade = T,
                  title=paste("Parameter information for task:",as.character(values$tasks_dtm[selectedRow,"task id"])),
                  DT::dataTableOutput(outputId = "more_details_dynamic_topic_table")
      )
    )
  }
})
...
#'   values$dtm_selected_row: selected row from topic model result list
#'   values$tasks_dtm: topic model tasks
output$more_details_dynamic_topic_table<-DT::renderDataTable({
  validate(
    need(values$dtm_selected_row>0,message=F)
  )
  data<-isolate(values$tasks_dtm[values$dtm_selected_row,,drop=F])
  data<-t(data)
