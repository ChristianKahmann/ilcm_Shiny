...
output$Vector_Results <- renderDataTable({
  ...
  isolate(values$reload_vector_result<-FALSE)
  files <- list.files("collections/results/vector-space-representation//")
  validate(
  ...
  if(dim(parameters)[1]>0){
    ...
    data_finished<-data_finished[,c("task id","collection","creation.time","class.existing.model","w2v.number.of.vectors",
  ...
  data_finished<-cbind(data_finished,Delete = shinyInput(
    shinyBS::bsButton,
    dim(data_finished)[1],
    'delete_button_vector_results_',
    label = "",
    size="small",
    style="danger",
    icon=icon("trash"),
    onclick = 'Shiny.onInputChange(\"delete_vector_results\",  this.id)'
  ))
  data_finished<-cbind(data_finished,More_Details = shinyInput(
    shinyBS::bsButton,
    dim(data_finished)[1],
    'more_details_button_vector_results_',
    label = "",
    size="small",
    style="primary",
    icon=icon("info"),
...
observeEvent(input$more_details_vector_results,{
  ...
  if(selectedRow>0){
    values$vector_selected_row<-selectedRow
    isolate(shinyjs::runjs('Shiny.onInputChange(\"more_details_vector_results\",  "more_details_button_vector_results_0")'))
    showModal(
      modalDialog(easyClose = T,fade = T,
                  title=paste("Parameter information for task:",as.character(values$tasks_vector[selectedRow,"task id"])),
