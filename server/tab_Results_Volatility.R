...
output$Volat_Results <- renderDataTable({
  ...
  if(dim(parameters)[1]>0){
    ...
    data_finished<-data_finished[,c("task id","collection","creation.time","va_method","va_timeintervall","va_history","cooc.significance.measure",
                                    "stopwords","Pruning.Term","min.term","max.term","POS.Types","NER.Tags")]
  }
  else{
    values$tasks_volat<-data_finished
  }
