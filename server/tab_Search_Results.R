...
output$search_results_datatable<-DT::renderDataTable({
  ...
  for(d in isolate(input$dataset)){
    ...
    ind_tmp[which(is.na(ind_tmp[,1:7]),arr.ind = T)]<-""
  ...
  if(values$custom==TRUE){
    remove_existing_checkboxes(1:10)
    data<-datatable(data.frame(data,keep=shinyInput_checkbox(checkboxInput,dim(data)[1],"cbox_",values=!(data[,1]%in%isolate(values$delete_documents)),label=NULL))
                    ,selection = "single",rownames = FALSE,escape = F,options = list(
...
$(".sorting_asc").on("click",function() {
      ...
      data<-datatable(data.frame(data,keep=shinyInput_checkbox(checkboxInput,dim(data)[1],"cbox_",values=!(data[,1]%in%isolate(values$delete_documents)),label=NULL)),selection = "single",rownames = FALSE,escape = F,options = list(
        preDrawCallback = JS('function() { Shiny.unbindAll(this.api().table().node()); }'),
        drawCallback = JS('function() { Shiny.bindAll(this.api().table().node()); } '),
        dom="t",
        order=list(3,"desc"),
  ...
  if (length(s)) {
    ...
    if(!"dataset"%in%colnames(values$Search_Results)){
      ...
      if(!"id_doc"%in%colnames(values$Search_Results)){
        ...
        values$new<-NULL
        values$annotations_show<-matrix(c(0),0,12)
        values$annos<-NULL
      }
    }
  }
...
observe({
  ...
  a<-do.call(rbind,a)
  docs_del<-isolate(values$Search_Results[which(a==F),1])
  print(docs_del)
  isolate(values$delete_documents<-setdiff(values$delete_documents,values$Search_Results[,1]))
  isolate(values$delete_documents<-c(isolate(values$delete_documents),isolate(values$Search_Results[which(a==F),1])))
})
