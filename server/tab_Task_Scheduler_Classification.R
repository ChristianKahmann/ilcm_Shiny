...
output$Analysis_Parameter_CL<-renderUI({
  tagList(
    ...
    fluidRow(
      column(2,
             selectizeInput(inputId = "CL_Context_Unit",label="Context Unit",choices=c("Sentence"))%>%
               shinyInput_label_embed(
                 shiny_iconlink() %>%
                   bs_embed_popover(
      ...
      column(3,
             conditionalPanel(condition = 'input.CL_Context_Unit=="Sentence"',
                              selectizeInput(inputId = "CL_Mode",label="Mode",choices=c("Produce 50 new active learning examples","Active learning on whole documents","Evaluate Training Set","Classify on entire collection"))
             ),
             conditionalPanel(condition = 'input.CL_Context_Unit!="Sentence"',
                              selectizeInput(inputId = "CL_Mode",label="Mode",choices=c("Produce 50 new active learning examples","Evaluate Training Set","Classify on entire collection"))
             )
      ),
             ...
             conditionalPanel(condition='input.CL_Mode=="Produce 50 new active learning examples"',
                              checkboxInput(inputId = "CL_use_dict",label = "Use Dictionary?",value = FALSE)%>%
                                shinyInput_label_embed(
                                  shiny_iconlink() %>%
                                    bs_embed_popover(
                                      title = "Do you wihs to use a dictioanry to create the active learning examples. The specified dictionaries column names and the categories for that project have to match exactly.",
                                      placement = "right",
                                      html="true"
                                    )
                                )
...
observeEvent(input$CL_Submit_Script,{
...
observeEvent(input$CL_pruning_continue,ignoreInit = T,{
  validate(
