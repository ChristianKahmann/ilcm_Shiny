...
output$Analysis_Parameter_TM<-renderUI({
  tagList(
    ...
    fluidRow(
      column(2,
             checkboxInput(inputId="TM_use_fixed_vocab",label="use fixed vocabulary?",value=F)   %>%
      ...
      column(2,
             conditionalPanel(condition = 'input.TM_use_fixed_vocab==true',
                              selectInput(inputId="TM_fixed_vocab",label="found vocabularies",choices=list.files("collections/vocabularies/",full.names = F),multiple=F)
             )
      )
    ),
    fluidRow(
      ...
      column(2,
             conditionalPanel(condition='input.TM_use_fixed_vocab==false',
    ...
    fluidRow(
      column(2,
             checkboxInput(inputId="TM_use_precalculated_topic_model",label="use pre-calculated topic model",value=F)
      ),
      column(3,
             conditionalPanel(condition = 'input.TM_use_precalculated_topic_model==true',
                              selectInput(inputId="TM_precalculated_topic_model",label="found topic models",choices=list.files("collections/results/topic-model/",full.names = F),multiple=F)
             )
      )
    ),
    conditionalPanel(condition='input.TM_use_precalculated_topic_model==false',
                     fluidRow(
                       column(1,
                     ...
                     conditionalPanel(condition='input.TM_use_fixed_vocab==false',
                                      fluidRow(
                                        column(1,
                                               selectInput(inputId = "TM_POS_TYPES",label = "Include POS-Types",
                                        ...
                                        column(1,
                                               selectInput(inputId = "TM_ENTITY_TYPES",label = " Include NER-Tags",
                                        ...
                                        column(1,
                                               selectInput(inputId = "TM_POS_TYPES_exclude",label = "Exclude POS-Types",
                                        ...
                                        column(1,
                                               selectInput(inputId = "TM_ENTITY_TYPES_exclude",label = "Exclude NER-Tags",
                                                           choices =c("all","PER","ORG","GPE","PRODUCT","NORP","FACILITY","LOC","EVENT","WORK_OF_ART","LAW",
...
output$TM_whitelist_UI<-renderUI({
  if(length(list.files("collections/whitelists/"))==0){
    ...
    return(shinyjs::hidden(
      prettyRadioButtons(inputId = "TM_whitelist",label = "Whitelists",
...
output$TM_blacklist_UI<-renderUI({
  if(length(list.files("collections/blacklists/"))==0){
    ...
    return(shinyjs::hidden(
      prettyRadioButtons(inputId = "TM_blacklist",label = "Blacklists",
...
observeEvent(input$TM_Submit_Script,{
  ...
  if(isFALSE(valid_pruning)){
    ...
    shinyWidgets::confirmSweetAlert(session = session,title = "Check vocabulary",text = HTML("You chose to use a predefined vocabulary. It seems this vocabulary is not present."),html=T,inputId="TM_pruning_continue",
                                    type="warning",closeOnClickOutside = T,btn_labels = c("Change Settings","Continue anyway!"))
  }
  else{
    ...
    parameters<-list(collection=input$collection_selected,
                     ...
                     reduce_POS_exclude=input$TM_POS_TYPES_exclude,
                     reduce_NER=input$TM_ENTITY_TYPES,
                     reduce_NER_exclude=input$TM_ENTITY_TYPES_exclude,
...
#' depends on:
#'   input$TM_pruning_continue
#'   input$TM_termfreq_type
#'   input$TM_min_termfreq_c 
#'   input$TM_max_termfreq_c
#'   input$TM_min_docfreq_c
#'   input$TM_max_docfreq_c
#'   input$TM_min_termfreq_r
#'   input$TM_max_termfreq_r:
#'   input$TM_min_docfreq_r:
#'   input$TM_max_docfreq_r
#'   input$TM_min_termfreq_p
#'   input$TM_max_termfreq_p
#'   input$TM_min_termfreq_q
#'   input$TM_max_termfreq_q
#'   input$TM_min_termfreq_q
#'   input$TM_max_termfreq_q
...
#'   input$TM_min_char:
#'   input$TM_ngram:
#'   input$TM_remove_stopwords:
#'   input$TM_lowercase:
#'   input$TM_remove_numbers:
#'   input$TM_remove_numbers_all:
#'   input$TM_remove_punctuation:
#'   input$TM_remove_hyphenation:
#'   input$TM_min_Cooc_Freq:
#'   input$TM_remove_custom:
#'   input$TM_consolidate_entities:
#'   input$TM_blacklist:
#'   input$TM_POS_TYPES:
#'   input$TM_POS_TYPES_exclude:
#'   input$TM_ENTITY_TYPES:
#'   input$TM_ENTITY_TYPES_exclude:
#'   input$TM_docfreq_type:
#'   input$TM_keep_custom:
#'   input$TM_use_custom_blacklist:
#'   input$TM_use_custom_whitelist:
#'   input$TM_whitelist:
#'   input$TM_whitelist_expand:
#'   input$TM_whitelist_only:
#'   input$TM_number_of_topics:
#'   input$TM_alpha:
#'   input$TM_method:
#'   input$TM_detailed_meta_dist:
#'   input$TM_use_fixed_vocab:
#'   input$TM_fixed_vocab:
#'   input$TM_use_precalculated_topic_model:
#'   input$TM_precalculated_topic_model:
#'   input$stm_prevalenceFormula:
#'   input$stm_contentFormula:
#'   input$stm_init_type:
#'   input$stm_max_em_its:
#'   input$stm_emtol:
#'   input$stm_LDAbeta:
#'   input$stm_interactions:
#'   input$stm_ngroups:
#'   input$stm_gamma_prior:
#'   input$stm_sigma_prior:
#'   input$stm_kappa_prior:
